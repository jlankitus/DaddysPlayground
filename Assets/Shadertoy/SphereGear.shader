...
float sdCross( in float2 p, in float2 b, float r ) 
{
	...
	float2  q = p - b;
    float k = max(q.y,q.x);
    float2  w = (k>0.0) ? q : vec2(b.y-p.x,-k);
...
{
    float2 k1 = vec2(r2,he);
    float2 k2 = vec2(r2-r1,2.0*he);

	p.x = abs(p.x);
    float2 ca = vec2(max(0.0,p.x-((p.y<0.0)?r1:r2)), abs(p.y)-he);
...
float dents( in float2 q, in float tr, in float y )
{
    const float an = 6.283185/12.0;
    float fa = (atan2(q.x,q.y)+an*0.5)/an;
    float sym = an*floor(fa);
    // float2 r = float4x4(cos(sym), -sin(sym), sin(sym), cos(sym)) * q;
    float2 r = float4(cos(sym), -sin(sym), sin(sym), cos(sym)) * q;
...
float4 gear(float3 q, float off, float time)
{
    ...
    // q.xz = float4x4(co,-si,si,co)*q.xz;
    q.xz = float4(co,-si,si,co)*q.xz;
    }
    
    q.y = abs(q.y);
    
    float an2 = 2.0*min(1.0-2.0*abs(frac(0.5+time/10.0)-0.5),1.0/2.0);
    float3 tr = min( 10.0*an2 - vec3(4.0,6.0,8.0),1.0);
    ...
    float2 w = vec2( d3, abs(q.y-0.485)-0.005*tr.y );
    ...
    // float4 ret = vec4(d, q.xzy);
    float4 ret = vec4(d, q.x, q.y, q.z);
    return ret;
}
...
float4 map( in float3 p, float time )
{
    // center sphere
    float4 d = vec4( sdSphere(p,0.12), p, 0, 0 );
    
    // gears. There are 18, but we only evaluate 4    
    float3 qx = vec3(rot(p.zy),p.x); if(abs(qx.x)>abs(qx.y)) qx=qx.zxy;
    float3 qy = vec3(rot(p.xz),p.y); if(abs(qy.x)>abs(qy.y)) qy=qy.zxy;
    float3 qz = vec3(rot(p.yx),p.z); if(abs(qz.x)>abs(qz.y)) qz=qz.zxy;
...
for( int i=ZERO; i<4; i++ )
    {
        float3 e = 0.5773*(2.0*vec3((((i+3)>>1)&1),((i>>1)&1),(i&1))-1.0);
...
float calcSoftshadow( in float3 ro, in float3 rd, in float k, in float time )
{
    float res = 1.0;
...
float4 intersect( in float3 ro, in float3 rd, in float time )
{
    float4 res = vec4(-1.0);
...
for( int i=0; i<128 && t<tminmax.y; i++ )
        {
            ...
            if( h.x<0.001 ) { res=vec4(t,h.y, h.z, h.w); break; }
            t += h.x;
        }
    }
...
for( int n=ZERO; n<AA; n++ )
    {
        // pixel coordinates
        float2 o = vec2(float(m),float(n)) / float(AA) - 0.5;
        float2 p = (2.0*(vertex_output.uv+o)-1)/1;
        float d = 0.5*sin(vertex_output.uv.x*147.0)*sin(vertex_output.uv.y*131.0);
        float time = _Time.y - 0.5*(1.0/24.0)*(float(m*AA+n)+d)/float(AA*AA-1);
        #else    
        float2 p = (2.0*vertex_output.uv-1)/1;
        float time = _Time.y;
        #endif

	    // camera	
        float an = 6.2831*time/40.0;
        float3 ta = vec3( 0.0, 0.0, 0.0 );
        float3 ro = ta + vec3( 1.3*cos(an), 0.5, 1.2*sin(an) );
        ...
        // ray direction
        float fl = 2.0;
        float3 rd = ca * 2; // normalize( vec3(p,fl,0) );

        // background
        float3 col = vec3(1.0+rd.y)*0.03;
        ...
        if( tuvw.x>0.0 )
        {
            ...
            float3 te = 0.5*tex2D( _MainTex, tuvw.yz*2.0 ).xyz+
                      0.5*tex2D( _MainTex, tuvw.yw*1.0 ).xyz;
            ...
            // top
            {
            float3  lig = normalize(vec3(0.8,0.2,0.6));
            ...
            float sha = occ;//0.2*occ + 0.8*calcSoftshadow( pos+0.001*vec3(0,1,0), vec3(0.0,1.0,0.0), 3.0, time );
    ...
    // cheap dithering
    tot += sin(vertex_output.uv.x*114.0)*sin(vertex_output.uv.y*211.1)/512.0;
